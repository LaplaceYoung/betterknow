...
import { Search, Flame, ChevronDown, Clock } from 'lucide-react'
...
export default function Marketplace() {
  ...
  const shown = useMemo(() => (list ?? []).filter((c) => !q || c.courseTitle.toLowerCase().includes(q.toLowerCase()) || c.courseDescription.toLowerCase().includes(q.toLowerCase())), [list, q])
  ...
  return (
    <div className="mx-auto max-w-[1180px] px-8 pb-16">
      ...
      <div className="flex items-center justify-between mt-5 mb-4">
        <h2 className="text-[16px] font-semibold">{subject === 'all' ? '全部课程' : SUBJECT_LABEL[subject]} <span className="text-[12px] text-[#8a8a90] font-normal ml-1">{list ? `${shown.length} 门课程` : ''}</span></h2>
        <button className="hk-pill text-[12px]">排序方式 <span className="font-medium">推荐排序</span> <ChevronDown size={12} /></button>
      </div>
