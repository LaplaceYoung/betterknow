          ...
          <div className="max-w-[774px] mx-auto px-6 pb-6 pointer-events-auto">
            <div className="hk-composer p-3.5">
              ...
              <div className="flex items-center gap-2 mt-2">
                <button type="button" onClick={() => followupFileRef.current?.click()} className="response-add-button" aria-label="上传附件" title="上传图片或文件"><Plus size={14} /></button>
                <span className="hk-pill h-7 text-[12px]"><Languages size={11} /> {language === 'zh' ? '中文' : language === 'en' ? 'English' : '한국어'}</span>
                <div className="ml-auto flex items-center gap-2">
                  <button className="response-tools-button" onClick={async () => { const token = localStorage.getItem('access_token') ?? ''; const r = await fetch('/api/v1/share_record/share_records', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` }, body: JSON.stringify({ type: 'conversation', shared_object_id: convId, shared_with: { share_to_everyone: true } }) }).then((x) => x.json() as Promise<{ shared_url?: string }>).catch((): { shared_url?: string } => ({})); nav(r.shared_url ?? `/share/c/${convId}`) }}><span className="response-tools-button-icon"><Share2 size={16} /></span><span className="response-tools-button-text">分享对话</span></button>
                  <button className="response-tools-button"><span className="response-tools-button-icon"><LifeBuoy size={16} /></span><span className="response-tools-button-text">遇到问题？</span></button>
