...
In the softmax regression section, we introduced
softmax regression,
implementing the algorithm from scratch
(the softmax-from-scratch section) and using high-level APIs
(the concise softmax section). This allowed us to
...
the linear model subsection as
...
*Figure omitted.* An MLP with a hidden layer of five hidden units.
...
when we denoted a rowwise operation in the softmax vectorization subsection.
...
*Figure omitted.* The ReLU activation function.
...
*Figure omitted.* Derivative of the ReLU function.
...
Nonetheless sigmoids are important. In later chapters (e.g., the LSTM section) on recurrent neural networks,
...
*Figure omitted.* The sigmoid activation function.
...
*Figure omitted.* Derivative of the sigmoid function.
...
*Figure omitted.* The tanh activation function.
...
*Figure omitted.* Derivative of the tanh function.
