...
(5+9)*(15-7)
```
There has been a change in the division operator between Python 2
and Python 3.  In Python 3, the result of this division is a
...
`input` that gets input from the keyboard^[In Python 2, this function was named `raw_input`.].
...
messages are `SyntaxError: invalid syntax` which is not very informative.
