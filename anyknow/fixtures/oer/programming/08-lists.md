...
Course-aliases: 程序设计, Python, 程序设计基础
Extract: UTF-8 plain-text extract of Python for Everybody by Charles R. Severance for the simo know local teaching knowledge base.
Based on Think Python by Allen B. Downey. Figures are omitted or replaced with captions.
...
it, removing the first and last elements, and returns `None`. Then write a function
...
you produce the list of all the words that Shakespeare used? Would you
download all his work, read it and track all unique words by hand?

Let's use Python to achieve that instead. List all unique words, sorted in alphabetical
order, that are stored in a file `romeo.txt` containing a subset of Shakespeare's work.
...
`split` function. For each word, check to see if the word is already in the list
of unique words. If the word is not in the list of unique words, add it to the list.
...
you find a line that starts with "From", you will split the line into
