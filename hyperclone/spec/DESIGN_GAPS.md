...
**第三十六批（生成后评分 + 欢迎卡变体）**
- **生成页评分条**：新增 `GenerationRating`，复用课程页那套评分内件，定位按线上 `.course-rating-bar--generation`（`bottom: calc(clamp(44px,6vh,76px) - 52px)`、水平居中、绝对定位），挂在课程生成完成卡片之后；产物里规则已就位。
  - **诚实说明**：这一条的**挂载没在复刻站实测到**。原因有两个：首页「打造课程」的自动化填充没能让发送键变成可用态（脚本写值没触发受控状态），而直接访问 `/response/course-generation/new` 那条路径本轮几次尝试都停在问卷/结构确认之前没跑到完成态（此前的一次生成确实跑到过完成并落到课程页）。所以：内件与课程页评分条同源且已验证，定位规则来自线上样式表，但「生成完成后就地出现」这一步是**未实测**状态。
- **欢迎卡下一步标识按类型上色**：`.cj-welcome-next-kind` 的四种变体（默认/`--practice`/`--project`/`--exam`）落地，按首个节点的 `session_type` 选择；实测讲座类型渲染为 `--learn`（`rgb(238,242,248)` / `rgb(76,102,150)` / 边框 `rgba(76,102,150,.16)`）。
